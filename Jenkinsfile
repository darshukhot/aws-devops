        ...
        stage('Deploy to Web Server') {
            steps {
                sshagent(credentials: ['webserver-ssh']) {
                    sh '''
                         ssh -o StrictHostKeyChecking=no ubuntu@54.92.166.168 'sudo mkdir -p /tmp/html && sudo chown ubuntu:ubuntu /tmp/html'
                          scp -o StrictHostKeyChecking=no html_package.tar.gz ubuntu@54.92.166.168:/tmp/html/
